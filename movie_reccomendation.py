...
tk.Label(root, text="Enter a movie name or genre and get similar suggestions 🍿", 
...
def recommend_movies():
    ...
    if not query:
        messagebox.showwarning("⚠️ Input Error", "Please enter a movie name or genre!")
        return
    ...
    time_lbl.config(
        text=f"✅ Found {len(results)} matches for '{query}' | Time: {round(end - start, 4)} sec (O(n log n))"
    )
...
def show_poster():
    selected = tree.focus()
    if not selected:
        messagebox.showwarning("⚠️ No Selection", "Please select a movie first!")
        return
    ...
    try:
        ...
        poster_lbl.config(image="", text="❌ Poster not found", fg="white", bg="#0B132B", font=("Arial", 12))
...
def show_trending():
    ...
    for i in tree.get_children():
        tree.delete(i)
    trending_lbl.config(text="🔥 Top 5 Trending Movies 🔥", fg="#FFD700", bg="#0B132B", font=("Arial", 14, "bold"))
    for _, m in data.iterrows():
        tree.insert("", "end", values=(m["title"], m["genres"], m["year"], m["language"], m["rating"]))

# ---- Buttons ----
tk.Button(frame, text="🎯 Recommend", command=recommend_movies, bg="#00C9A7", fg="black",
          font=("Arial", 10, "bold")).grid(row=0, column=2, padx=10)
tk.Button(frame, text="🧹 Clear", command=clear_all, bg="#1F4068", fg="white",
...
# ---- Poster Display ----
tk.Button(root, text="🎞 Show Poster", command=show_poster, bg="#00C9A7",
